...
SERVER_NAME=`sed '/application.name/!d;s/.*=//' config/application.yml | tr -d '\r'`
SERVER_PORT=`sed '/server.port/!d;s/.*=//' config/application.yml | tr -d '\r'`
LOGS_FILE=`sed '/log.file.name/!d;s/.*=//' config/application.yml | tr -d '\r'`
...
STDOUT_FILE=$LOGS_DIR/system.log
...
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.lzx2005.application.Application > $STDOUT_FILE 2>&1 &
